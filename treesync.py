...
def endpoint_handler():
    if request.method == 'POST':
        ...
        for id in latest_emails:
            ...
            event_present = llmCall([f"""You are an agent that receives an email subject and its body, along with the current date and time. Your job is to figure out if the email contains an event scheduled in the future. For example, if the email asks the recepient if he wants to grab lunch tomorrow, the email contains an event scheduled in the future. If the email is telling the recepient about a news event that happened yesterday, the email does not contain an event scheduled in the future. Your job is to return either 1 or 0 depending on this problem. Output 1 if the email contains an event scheduled in the future. Output 0 if the email does not contain an event scheduled in the future. Only output either 1 or 0. Do not output anything else and do not explain your answer.""", f"""Here is the email subject: {email_subject}, here is the email body: {email_body}, here is the current date and time: {email_date}"""])
            print(event_present)
            if event_present is not None and '1' in event_present:
                ...
                if scheduled_event and scheduled_event not in prev_events:
